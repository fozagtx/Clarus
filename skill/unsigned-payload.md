...
| Step | Output |
|---|---|
| 1 | **PancakeSwap v2** swap URL (BNB → token, `chain=bsc`, `exactIn`, `utm_source=clarus`) |
| 2 | **Binance Web3 Wallet** deep link wrapping that URL (`bnc://app.binance.com/mp/app?appId=bnb1w4cqqe&startPagePath=/swap&startPageQuery=…`) |
| 3 | Print both URLs |
| 4 | **Clarus does not execute this swap. Sign in your own wallet or discard.** |
...
| Forbidden | Why |
|---|---|
| `ethers.Wallet`, `viem` account, privateKey, mnemonic, `eth_sendTransaction`, `eth_sendRawTransaction` | This skill never signs |
| Jupiter, Solana RPC, Telegram bot sends | BNB Chain + unsigned URLs only |
| Take-profit / stop-loss / DCA / "shave 10–20% at 2x" | No position management |
| Any sentence that says the agent bought, sold, or deployed capital | Operator is the signer |
